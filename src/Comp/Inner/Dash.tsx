...
//   return (
//     <SignedIn>
//       <div className="relative min-h-screen bg-[#1f1f1f] text-gray-300 px-6 py-4 overflow-hidden">
//         <Nav />

//         <motion.div
//           initial={{ scale: 0.8, opacity: 0.1 }}
//           animate={{ scale: 2.1, opacity: 0.8}}
//           transition={{ duration: 1.5, ease: "easeOut" }}
//           className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[12vw] font-extrabold text-[#fff] tracking-wide"
//         >
//           TRASH.
//         </motion.div>

//         <div ref={containerRef} className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 px-4 py-10">
//           {[...Array(6)].map((_, index) => {
...
//                 className="break-inside-avoid w-full mb-6 p-5 bg-[#121211] border border-[#383838] rounded-2xl shadow-3xl backdrop-blur-md cursor-grab active:cursor-grabbing overflow-hidden"
//               >
//                 <div className="flex items-center justify-between">
//                   <div className="flex items-center gap-3">
//                     <UserButton />
//                     <p className="text-gray-400 font-medium">~ {user?.firstName || "Unknown"}</p>
//                   </div>
//                   <button className="text-gray-500 hover:text-red-500 transition-all"><MdDeleteSweep /></button>
//                 </div>

//                 <h3 className="mt-3 font-bold text-gray-100 text-lg">{blogNames[index] || `Blog ${index + 1}`}:</h3>

//                 {contentType === 0 && <p className="text-gray-400 text-sm line-clamp-3">This is a text-based blog post.</p>}

//                 {contentType === 1 && (
//                   <img src={sampleImages[index % sampleImages.length]} alt="Blog" className="w-full mt-3 rounded-lg" />
//                 )}
...
//                     <Document
//                       file={samplePdf}
//                       className="border border-gray-700 rounded-md overflow-hidden shadow-md"
...
//                 <p className="text-xs text-gray-500 mt-3">Size: {Math.random().toFixed(2)} MB</p>
...
export default function Dashboard() {
  ...
  return (
    <SignedIn>
      <div className={`${darkMode ? "bg-gradient-to-b from-[#0d0d0d] to-[#1a1a2e] text-gray-300" : "bg-gradient-to-b from-[#f5f7fa] to-[#c3cfe2] text-gray-800"} relative  px-6 py-4 overflow-y-scroll transition-colors`}>        
        <Nav />
        ...
        <motion.div
          ...
          transition={{ duration: 1.5, ease: "easeOut" }}
          className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[12vw] font-extrabold bg-gradient-to-r from-[#7b2ff7] to-[#4c51bf] text-transparent bg-clip-text tracking-wide max-h-screen"
        >
          DOCS.
        </motion.div>
